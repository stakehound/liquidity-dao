...
interface Options {
    ...
    sort?: boolean;
    /** If set to `true`, the leaves and branches will be prefixed with 0x00 and 0x01 respectively */
    prefix?: boolean;
    recreate?: {
        leaves: TLeafHex[];
        layers: TLayerHex[];
...
type LayerObject = { [layer: string]: Buffer | LayerObject };
type THashAlgo = any;
type TValue = any;
type TLeaf = Buffer;
type TLayer = Buffer[];

type TLeafHex = string;
type TLayerHex = string[];

export const leafPrefix = Buffer.from("00", "hex");
export const branchPrefix = Buffer.from("01", "hex");
...
export class MerkleTree {
    ...
    constructor(leaves: any[], hashAlgorithm: any, options: Options = {}) {
        ...
        if (!!options.recreate) {
            const { leaves, layers } = options.recreate;
            this.leaves = leaves.map(this._bufferify);
            this.layers = layers.map((layer) => layer.map(this._bufferify));
            if (this.sortLeaves) {
                this.leaves = this.leaves.sort(Buffer.compare);
            }
        } else {
            if (this.isBitcoinTree && this.prefix) {
                throw new Error(
                    "MerkleTree: isBitcoinTree and prefix incompatible options"
                );
            }
            // if (!this.duplicateOdd && this.prefix) {
            //   // TODO: Maybe doesn't need to be duplicateOdd?
            //   throw new Error("MerkleTree: prefix must be used with duplicateOdd");
            // }

            this.leaves = leaves.map(this._bufferify);

            if (this.prefix) {
                this.leaves = this.leaves.map((l) => Buffer.concat([leafPrefix, l]));
            }

            if (this.hashLeaves) {
                this.leaves = this.leaves.map(this.hashAlgo);
            }
    ...
    private _createHashes(nodes: any[]) {
        while (nodes.length > 1) {
            ...
            for (let i = 0; i < nodes.length; i += 2) {
                ...
                if (this.sortPairs) {
                    combined = combined.sort(Buffer.compare);
                }

                if (this.prefix) {
                    combined.unshift(branchPrefix);
                }
     ...
     */
    getHexLayers(): string[][] {
        return this.layers.reduce((acc: string[][], layer, i) => {
            acc.push(layer.map((item) => this._bufferToHex(item)));
            return acc;
        }, []);
    }
    ...
    getProof(leaf: Buffer, index?: number): any[] {
        ...
        if (this.prefix) leaf = Buffer.concat([leafPrefix, leaf]);
        if (this.hashLeaves) leaf = this.hashAlgo(leaf);
        const proof = [];
    ...
    getMultiProof(tree?: any[], indices?: any[]): Buffer[] {
        if (!indices) {
            ...
            if (!indices.every((x) => typeof x === "number")) {
                ...
                let ids = els
                    .map((el) => this._bufferIndexOf(this.leaves, el))
        ...
        return this.getProofIndices(indices, this._log2((tree!.length / 2) | 0)).map(
            (index) => tree![index]
        );
    }
    ...
    getProofFlags(leaves: Buffer[], proofs: Buffer[]): boolean[] {
        ...
        for (let index = 0; index < this.layers.length; index++) {
            ...
            ids = ids.reduce((ids: number[], idx) => {
                const skipped = tested.includes(layer[idx]);
                if (!skipped) {
                    const pairElement = this._getPairNode(layer, idx);
                    const proofUsed =
                        proofs.includes(layer[idx]) || proofs.includes(pairElement!);
    ...
    verify(proof: any[], targetNode: Buffer, root: Buffer): boolean {
        // TODO: handle target node with prefix and hash leaves
        let hash = this._bufferify(targetNode);
        if (this.prefix) hash = Buffer.concat([leafPrefix, hash]);
        if (this.hashLeaves) hash = this.hashAlgo(hash);
    ...
    verifyMultiProof(
        ...
        for (const [index, proofitem] of this._zip(
            this.getProofIndices(indices, depth),
            proof
        )) {
        ...
        return (
            !indices.length || ({}.hasOwnProperty.call(tree, 1) && tree[1].equals(root))
        );
    }
    ...
    getLayersAsObject(): any {
        const layers: string[][] = this.getLayers().map((layer: Buffer[]) =>
            layer.map((value: Buffer) => value.toString("hex"))
        );
        const objs = [];
        for (let i = 0; i < layers.length; i++) {
            const arr = [];
            for (let j = 0; j < layers[i].length; j++) {
                const obj: LayerObject = { [layers[i][j]]: null } as any;
                if (objs.length) {
                    obj[layers[i][j]] = {};
                    const a = objs.shift()!;
                    const akey = Object.keys(a)[0];
                    (<LayerObject>obj[layers[i][j]])[akey] = a[akey];
                    if (objs.length) {
                        const b = objs.shift();
                        const bkey = Object.keys(b!)[0];
                        (<LayerObject>obj[layers[i][j]])[bkey] = b![bkey];
                    }
                }
    ...
    private _toTreeString(): string {
        ...
        return (<any>treeify).asTree(obj, true);
    }
     ...
     *```
     */
    static getMultiProof(tree: Buffer[], indices: number[], hashAlg: any): Buffer[] {
        const t = new MerkleTree([], hashAlg);
     ...
     *```
     */
    private _getPairNode(layer: Buffer[], idx: number): Buffer | null {
    ...
    static bufferify(value: any): Buffer {
        if (!Buffer.isBuffer(value)) {
            if (MerkleTree.isHexString(value)) {
     ...
     *```
     */
    private _bufferifyFn<T, U>(f: (value: T) => Buffer | string): (value: T) => Buffer {
        return (value: T) => {
            const v = f(value);
            ...
            throw new Error('removed crypto-js support')
        };
    }
