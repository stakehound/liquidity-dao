...
const force_propose = async (context: StakehoundContext, proposer: Signer) => {
    ...
    logger.info(
        `Force rewards: Proposed cycle ${merkle.merkleRewards.cycle} merkle root ${merkle.merkleRewards.merkleRoot}`
    );
    ...
    await tx.wait(1).then((tx) => logger.info(`ForceRewards: gasPaid: ${tx.gasUsed}`));
...
const init_rewards = async (context: StakehoundContext, proposer: Signer) => {
    ...
    await tx.wait(1).then((tx) => logger.info(`Init Propose: gasPaid: ${tx.gasUsed}`));
    const { seven, thirty } = await wait_for_confirmations(
        context.provider,
        tx.hash,
        "Init"
...
const bump_rewards = async (context: StakehoundContext, proposer: Signer) => {
    ...
    await tx.wait(1).then((tx) => logger.info(`Bump: gasPaid: ${tx.gasUsed}`));
    const { seven, thirty } = await wait_for_confirmations(
        context.provider,
        tx.hash,
        "Bump"
...
const approve_rewards = async (context: StakehoundContext, approver: Signer) => {
    ...
    await tx.wait(1).then((tx) => logger.info(`Approve: gasPaid: ${tx.gasUsed}`));
...
const force_approve = async (context: StakehoundContext, approver: Signer) => {
    ...
    await tx.wait(1).then((tx) => logger.info(`Force Approve: gasPaid: ${tx.gasUsed}`));
    const { seven, thirty } = await wait_for_confirmations(
        context.provider,
        tx.hash,
        "Force Approve"
