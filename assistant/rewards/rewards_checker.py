...
from assistant.rewards.StakehoundGeyserMock import StakehoundGeyserMock
...
import json
import brownie
from config.stakehound_config import stakehound_config, globalStartTime
...
def get_distributed_in_range(key, geyser, startBlock, endBlock):
    ...
    geyserMock = StakehoundGeyserMock(key)
...
def getExpectedDistributionInRange(stakehound: StakehoundSystem, startBlock, endBlock):
    distributions = {}
...
def diff_rewards(
    stakehound: StakehoundSystem, before_file, after_file,
):
...
def compare_rewards(
    ...
    totalExpected = stakehound_config.geyserParams.unlockSchedules.test[0].amount
    ...
    expectedGains = getExpectedDistributionInRange(stakehound, startBlock, endBlock)

    # Total claims must only increase
    sum_before = sum_claims(before)
    sum_after = sum_claims(after)
    sanitySum = Wei("2500000 ether")
...
def push_rewards(stakehound: StakehoundSystem, afterContentHash):
    with open("rewards-1-" + afterContentHash + ".json") as f:
        after_file = json.load(f)

    upload("rewards-1-" + afterContentHash + ".json")
    stakehound.multiplexer.proposeRoot(
        after_file["merkleRoot"],
        afterContentHash,
        after_file["cycle"],
        {"from": stakehound.keeper},
    ),

    stakehound.multiplexer.approveRoot(
        after_file["merkleRoot"],
        afterContentHash,
        after_file["cycle"],
        {"from": stakehound.guardian},
    ),


def test_claims(stakehound: StakehoundSystem, startBlock, endBlock, before_file, after_file):
    before = before_file["claims"]
    ...
    total_claimed = 0
    for user, claim in claims.items():
        claimed = stakehound.multiplexer.getClaimedFor(user, [stakehound.token.address])[1][0]
        print("Claim: ", user, claimed, claim["cumulativeAmounts"])
        if claimed == int(claim["cumulativeAmounts"][0]):
            print("SKIP!", user)
            continue
        accounts.at(user, force=True)
        pre = stakehound.token.balanceOf(user)
        tx = stakehound.multiplexer.claim(
            claim["tokens"],
        ...
        print(tx.events)
        post = stakehound.token.balanceOf(user)
        diff = post - pre
